function attachSolutionButtonListeners_question1(button) {
    button.addEventListener('click', function() {
        ...
        if (selectedAnswer && selectedAnswer.value === 'Admin1 and Admin3 only') {
            document.querySelector('input[name="question1"][value="Admin1 and Admin3 only"]').parentElement.classList.add('highlight');
        } else {
            document.querySelector('input[name="question1"][value="Admin1 and Admin3 only"]').parentElement.classList.add('incorrect');
            allCorrect = false;
        }

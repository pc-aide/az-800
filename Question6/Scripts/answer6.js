function attachSolutionButtonListeners_question6(button) {
    button.addEventListener('click', function() {
        ...
        if (selectedAnswer && selectedAnswer.value === 'No') {
            document.querySelector('input[name="question6"][value="No"]').parentElement.classList.add('highlight');
        } else {
            if (selectedAnswer) {
                document.querySelector('input[name="question6"][value="' + selectedAnswer.value + '"]').parentElement.classList.add('incorrect');
            }
            allCorrect = false;
        }
